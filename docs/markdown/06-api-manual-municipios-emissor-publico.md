# 06 Api Manual Municipios Emissor Publico

*Converted from: 06-api-manual-municipios-emissor-publico.pdf*
...
Gerenciamento Municipal no Sistema Nacional NFS-e – API



### Images from Page 1

![Image](images/06-api-manual-municipios-emissor-publico/page1_img1.png)
...
relacionadas com parâmetros municipais, DPS, API NFS-e e eventos.
Destinação
...
1.  Gerenciamento Municipal no Sistema Nacional NFS-e – API
Visão Geral
O gerenciamento municipal via API no Sistema Nacional NFS-e abrange um conjunto de
funcionalidades básicas para que o município desenvolva clientes para comunicação com o as API
disponíveis e possa integrar seus sistemas próprios de forma a automatizar o fluxo de informações
com o sistema nacional. Abaixo seguem as especificações das APIs atualmente disponíveis para
integração com o Sistema Nacional NFS-e.
...
necessárias da DPS para emissão e envio à Sefin Nacional NFS-e, que a validará para gerar a NFS-
e.
1.1.2.  Métodos
...
f) POST
–
/parametros_municipais/{codigoMunicipio}/beneficiomunicipal/
{idManut}
Realiza manutenção em um benefício municipal já existente em um município.
g) POST – /parametros_municipais/{codigoMunicipio}/retencoes/ {idManut}
Realiza manutenção em uma retenção já existente em um município.
h) POST
–
/parametros_municipais/{codigoMunicipio}/regimes_especiais/
{idManut}
Realiza manutenção em um regime especial de tributação já existente em um município.
1.2.  API NFS-e
1.2.1.  Descrição
Possui um serviço:
•
...
1.5.  Ambiente de produção restrita
